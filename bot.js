...
const devisSigners = new Map(); // Track qui a signé
...
async function generatePhotoEmbed(guild) {
  ...
  for (const userId in photoStatuses) {
    try { const member = await guild.members.fetch(userId); const name = member.nickname || member.user.username; desc += `• **${name}** → ${photoStatuses[userId]}\n`; } 
...
async function generateModelEmbed(guild) {
  ...
  for (const userId in modelStatuses) {
    try { const member = await guild.members.fetch(userId); const name = member.nickname || member.user.username; desc += `• **${name}** → ${modelStatuses[userId]}\n`; } 
...
function generateDashboardEmbed() {
  ...
  return new EmbedBuilder().setTitle("📊 Dashboard Global").setColor("#2f3136")
    .addFields({ name: "📸 Photographes actifs", value: `${p}`, inline: true }, { name: "👠 Modèles actifs", value: `${m}`, inline: true }).setTimestamp();
}
...
async function updatePanel(channelId, embed, key) {
  ...
  if (!msg) { msg = await channel.send({ embeds: [embed], components: key !== "dashboardMessageId" ? [dispoButtons] : [] }); panels[key] = msg.id; savePanels(panels); }
...
// READY
client.once("ready", async () => { console.log("✅ Bot prêt"); await refreshAll(); });

// DELETE PANEL
client.on("messageDelete", async msg => { if ([getPanels().photoMessageId,getPanels().modelMessageId,getPanels().dashboardMessageId].includes(msg.id)) await refreshAll(); });
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "watermark") {
    ...
    try {
      ...
      const wMarkBuffer = await sharp(path.join(__dirname, logo==="2"?"watermark2.png":logo==="3"?"watermark3.png":"watermark.png")).resize({width:logoWidth}).png().toBuffer();
      const wMeta = await sharp(wMarkBuffer).metadata();
      const marginX=Math.floor(meta.width*0.01); const marginY=Math.floor(meta.height*0.01);
      let top=0, left=0;
      ...
      const out = await img.composite([{ input: wMarkBuffer, top: Math.round(top), left: Math.round(left), blend:"over" }]).toBuffer();
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "devis") {
    await interaction.deferReply();
    const data = { client: interaction.options.getString('client'), telephone: interaction.options.getString('telephone'), photos: interaction.options.getInteger('photos'), description: interaction.options.getString('description'), prix: interaction.options.getInteger('prix') };
    const id = Date.now().toString(); devisCache.set(id,data);

    const canvas = createCanvas(800,1000); const ctx = canvas.getContext('2d');
    ...
    ctx.fillStyle="#ffffff"; ctx.fillRect(40,140,720,140); ctx.strokeStyle="#ddd"; ctx.strokeRect(40,140,720,140);
    ...
    ctx.fillStyle="#ffffff"; ctx.fillRect(40,320,720,350); ctx.strokeRect(40,320,720,350);
    ...
    for (let word of data.description.split(" ")){
      const testLine=line+word+" "; if(ctx.measureText(testLine).width>680){ ctx.fillText(line,60,y); line=word+" "; y+=28; }else line=testLine;
  ...
  if(interaction.isButton() && interaction.customId.startsWith("sign_")){
    ...
    for(let word of data.description.split(" ")){ const testLine=line+word+" "; if(ctx.measureText(testLine).width>680){ ctx.fillText(line,60,y); line=word+" "; y+=28; }else line=testLine; }
    ctx.fillText(line,60,y);
  ...
  if(interaction.isButton()){
    ...
    const member=interaction.member; const status=interaction.customId==="dispo_on"?"🟢":"🔴";
    if(interaction.channelId===PHOTO_CHANNEL_ID){ if(!member.roles.cache.some(r=>r.name===PHOTO_ROLE)) return interaction.editReply("❌ Tu n'es pas photographe."); photoStatuses[userId]=status; }
    else if(interaction.channelId===MODEL_CHANNEL_ID){ if(!member.roles.cache.some(r=>r.name===MODEL_ROLE)) return interaction.editReply("❌ Tu n'es pas modèle."); modelStatuses[userId]=status; }
    else return;
    saveStatuses({photoStatuses,modelStatuses}); await refreshAll(); return interaction.editReply("✅ Statut mis à jour");
  }
